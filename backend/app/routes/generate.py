...
def preview_certificate():
    ...
    if template_type == 'tesda':
        ...
        html_parts = [
            ...
            ".tesda-field-row { display: flex; justify-content: space-between; width: 100%; margin-bottom: 14px; }",
            ".tesda-label { color: #000; font-weight: 600; width: 240px; text-align: left; }",
            ".tesda-value { color: #e2e8f0; flex: 1; text-align: left; word-break: break-word; }",
            ".nav-buttons { margin-top: 30px; display: flex; justify-content: center; gap: 20px; }",
            ".nav-buttons button { background-color: #a361ef; border: none; padding: 10px 20px; border-radius: 8px; color: white; cursor: pointer; font-weight: bold; }",
            ".nav-buttons button:disabled { background-color: #888; cursor: not-allowed; }",
            "</style></head><body><div class='container'><h2 style='color:white;margin-bottom:30px;'>TESDA Data Preview</h2>"
        ]

        for index, row in enumerate(rows):
            name = get_value(row, "NAME") or "No Name"
            html_parts.append(f"<div class='tesda-card{' active' if index == 0 else ''}' id='card-{index}'>")
